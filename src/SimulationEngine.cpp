    ...
    std::string SimulationEngine::startTransfer(const std::string& barcode, int originId, int destId, bool byTime,
                                                std::string& error, int quantity)
    {
        ...
        if (quantity == 0) {
            ...
            prod = origin->searchByBarcode(barcode);
        }
    ...
    /**
     * Snapshot thread-safe del estado actual. Incluye colas de cada sucursal y últimas transferencias completadas.
     */
